...
//        var params = gptneox_context_default_params()
    ...
    func sample(ctx: OpaquePointer!,
        ...
        let n_ctx = gpt2_n_ctx(ctx)
        
        // Auto params
        
        let top_k = top_k <= 0 ? gpt2_n_vocab(ctx) : top_k
        let repeat_last_n = repeat_last_n < 0 ? n_ctx : repeat_last_n
        
//        // Get logits and vocab size
//        let vocabSize = gpt2_n_vocab(ctx)
//        guard let logits = gpt2_get_logits(ctx) else {
//            print("LLaMa sample error logits nil")
//            return 0
//        }
//
//        // Create candidates
//        var candidates = Array<gpt2_token_data>()
//        for i in 0 ..< vocabSize {
//            candidates.append(gpt2_token_data(id: i, logit: logits[Int(i)], p: 0.0))
//        }
//        var candidates_p = gpt2_token_data_array(data: candidates.mutPtr, size: candidates.count, sorted: false)
        
        // Apply penalties
//        let nl_token = Int(gpt2_str_to_token(ctx, "\n"))
//        let nl_logit = logits[nl_token]
//        let last_n_repeat = min(min(Int32(last_n_tokens.count), repeat_last_n), n_ctx)
        
        
        
//        gpt2_sample_repetition_penalty(context, &candidates_p,
//                    last_n_tokens.mutPtr.advanced(by: last_n_tokens.count - Int(repeat_last_n)),
//                    Int(repeat_last_n), repeat_penalty)
//        gpt2_sample_frequency_and_presence_penalties(ctx, &candidates_p,
//                    last_n_tokens.mutPtr.advanced(by: last_n_tokens.count - Int(repeat_last_n)),
...
//        }
        ...
        if (last_n_tokens.count>0){
            let sampl = gpt2_sample_repeat(ctx,
                                               last_n_tokens,
                                               last_n_tokens.count,
                                               top_k, top_p, temp,
            ...
            let sampl = gpt2_sample(ctx, top_k, top_p, temp)
            return sampl
        }
...
//        }
    ...
    public override func predict(_ input: String, _ callback: ((String, Double) -> Bool) ) throws -> String {
        ...
        while totalLength > contextLength {
            ...
            past.removeFirst()
            gpt2_shift_kv_cache(context, forgetCount)
            // Update count vars
            nPast -= forgetCount
        ...
        while outputEnabled {
            ...
            if outputToken == gpt2_token_bos() {
                print("🤖 [BOS]")
                skipCallback = true
            }
            // Convert token to string and callback
            if !skipCallback, let cStr = gpt2_token_to_str(context, outputToken) {
            ...
            if outputEnabled {
                ...
                if gpt2_eval(context, [outputToken], 1, nPast, contextParams.numberOfThreads) != 0 {
                    throw ModelError.failedToEval
                }
                // Increment past count
                nPast += 1
//                 Check to see if we need to forget (create space in context)
    ...
    public func embeddings(_ input: String) throws -> [Float] {
        ...
        if gpt2_eval(context, inputs, Int32(inputs.count), Int32(0), contextParams.numberOfThreads) != 0 {
            throw ModelError.failedToEval
        }

        let embeddingsCount = Int(gpt2_n_embd(context))
        guard let embeddings = gpt2_get_embeddings(context) else {
            return []
        }
    ...
    public override func tokenize(_ input: String, bos: Bool = false, eos: Bool = false) -> [ModelToken] {
        ...
        var embeddings = Array<ModelToken>(repeating: gpt2_token(), count: input.utf8.count)
        let n = gpt2_tokenize(context, input, &embeddings, Int32(input.utf8.count), bos)
        assert(n >= 0)
        embeddings.removeSubrange(Int(n)..<embeddings.count)
        
        if eos {
            embeddings.append(gpt2_token_eos())
        }
