...
//        let test = test_fn()
    ...
    func sample(ctx: OpaquePointer!,
        ...
        let top_k = top_k <= 0 ? gpt_neox_n_vocab(ctx) : top_k
...
//        }
    ...
    public override func predict(_ input: String, _ callback: ((String, Double) -> Bool) ) throws -> String {
        ...
        while totalLength > contextLength {
            ...
            past.removeFirst()
            gpt_shift_kv_cache(context, forgetCount)
            // Update count vars
            nPast -= forgetCount
        ...
        while outputEnabled {
            ...
            if outputToken == gpt_neox_token_eos() {
                outputEnabled = false
                print("🤖 [EOS]")
                break
            }
            ...
            if outputToken == gpt_neox_token_bos() {
                print("🤖 [BOS]")
                skipCallback = true
            }
            // Convert token to string and callback
            if !skipCallback, let cStr = gpt_neox_token_to_str(context, outputToken) {
