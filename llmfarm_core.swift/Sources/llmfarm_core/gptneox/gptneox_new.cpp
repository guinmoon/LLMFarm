...
//        if (file_version != GPTNEOX_FILE_VERSION_GGML)
        uint32_t param = file.read_u32();
        if (param==1 || param==0){
            hparams.use_parallel_residual = param;
            hparams.ftype = (enum gptneox_ftype) file.read_u32();
        }else{
            hparams.ftype = (enum gptneox_ftype) param;
        }
       
        
        //ggml 2023-05-19 gpt-neox
...
bool gpt_neox_eval(
        ...
        const int n_threads,
        const int n_past,
        const std::vector<gptneox_vocab::id> & embd_inp,
